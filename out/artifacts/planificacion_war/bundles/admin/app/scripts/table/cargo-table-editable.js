...
var TableEditableCargos = function () {
    ...
    var btnClickSalvarForm = function () {
        ...
        if ($('#cargo-form').valid() && area_id !== "") {
            ...
            var peso = $('#peso').val();
            var basico = ($('#cargo-basico').prop('checked')) ? true : false;
            var unidad = ($('#cargo-unidad').prop('checked')) ? true : false;
            var direccion = ($('#cargo-direccion').prop('checked')) ? true : false;
            var direcciones = new Array();
            if (direccion) {
                direcciones = $('#direccion').val();
            ...
            $.ajax({
                type: cargo_id === "" ? "POST" : "PUT",
                url: cargo_id === "" ? "cargo/salvarCargo" : "cargo/guardarCargo/" + cargo_id,
                contentType: "application/json;charset=utf-8",
                dataType: "json",
                data: JSON.stringify({
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        ...
                        direccionesServer = response.cargo.direcciones;
                        direcciones = new Array();
                        var tam = direccionesServer.length;
                        for (i = 0; i < tam; i++)
                            direcciones.push(direccionesServer[i].gerencia_id);
                        select = $('#direccion').select2();
                        select.val(direcciones).trigger("change");
                        scopeCargo = angular.element("#cargo-basico").scope();
                        scopeCargo.jerarquia = response.cargo.basico ? "basico" : response.cargo.deArea ? "unidad" : response.cargo.direccion ? "direccion" : "basico";
                        scopeCargo.$apply();
