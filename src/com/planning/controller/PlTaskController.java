...
public class PlTaskController {
    ...
    public ModelAndView createPlTask(@PathVariable Plan plan, PlTaskFilter filtro) {
        filtro.setPlan(plan);
        List<PlTask> taskList = planTareaService.findAll(filtro);
    ...
    public ModelAndView eliminarTareaGrafico(@RequestParam Integer tareaId, @RequestParam Integer planId, ModelMap map) {
        try {
            ...
            map.put("success", true);
            map.put("message", "Se eliminó la tarea: " + plTask.getName());
        } catch (Exception e) {
    ...
    public ModelAndView salvarTareaPlan(@RequestBody PlTask planTarea, @PathVariable("planId") Plan plan, @PathVariable("tareaId") Task task, ModelMap map) {
        if (planTarea.getId() == null) {
            ...
            if (!planTarea.validarTranversal()) {
                map.put("success", false);
                map.put("error", "Si selecciona mas de un nivel de alerta deben ser consecutivos."); 
            ...
            StatusTask statusTask = statusTaskService.findOne(1);
            planTarea.setStatusTask(statusTask);
            ...
            if (!plan.isEjecucion()) {
                ...
                map.put("success", false);
                map.put("error", "No se puede importar la tarea porque este plan está en ejecución.");
                return new RestModelAndView(map);
            }
    ...
    private ModelAndView editarTareaPlan(PlTask tarea, Plan plan, ModelMap map) {
        ...
        boolean existeDiagrama = plan.getDiagrama() != null && !plan.getDiagrama().isEmpty(), eraHito = false, esHito = tarea.isHito();
        ...
        if (!agrupadas.isEmpty()) {
            ...
            if (!tasks.isEmpty() && existeDiagrama) {
                desagruparTareas(plTaskBd, plan, tasks);
            }
            if (existeDiagrama) {
                eliminarTareasModelo(plan, agrupadas);
                agruparTareas(plTaskBd, plan);
            }
        }
        ...
        eraHito = plTaskBd.isHito() && !tarea.isHito();
        final Set<Channel> channels = new HashSet<>();
        tarea.getChannels().forEach(channel -> {
            channels.add(channelService.findOne(channel.getId()));
        });
        Modelo modelo = null;
        tarea.validarTranversal();
        boolean eraTranversal = false;
        if (!tarea.isTranversal() && plTaskBd.isTranversal()) {
            plTaskBd.setTranversal(tarea.isTranversal());
            eraTranversal = true;
        }
        if (existeDiagrama) {
            modelo = mapeadorObjetos.readValue(plan.getDiagrama(), Modelo.class);
            ejecutarExistenciaModelo(plan, plTaskBd, position, criticalyLevelsNew, modelo, tarea.isTranversal(), existeAgrupado, eraTranversal, eraHito, esHito);
        }

        List<ChildTask> sucesorasHijas = childTaskService.findByFromAndIsChild(plTaskBd.getId(), true);
        Set<Tarea> sucesorasOld = new HashSet<>();
        ...
        for (ChildTask childTask : sucesorasHijas) {
            PlTask sucesora = planTareaService.findOne(childTask.getTo());
            Tarea tareaSucesora = new Tarea(sucesora);
            sucesorasOld.add(tareaSucesora);
            boolean match = sucesorasNuevas.parallelStream().noneMatch(tareaNew -> tareaNew.equals(tareaSucesora));
            if (match) {
    ...
    private void desagruparTareas(PlTask tareaDesagrupada, Plan plan, HashSet<PlTask> tasks) {
        if (plan.getDiagrama() != null && !plan.getDiagrama().isEmpty()) {
            ...
            ArrayList<Node> nodes = modelo.buscarNodo(tareaDesagrupada.getId());
            for (Node node : nodes) {
                node.clonarDatos(new Node(tareaDesagrupada));
                modelo.updateNode(node);
            }
            if (modelo.isModificado()) {
                String modeloNew = mapeadorObjetos.writeValueAsString(modelo);
                plan.setDiagrama(modeloNew);
    ...
    private void agruparTareas(PlTask task, Plan plan) {
        Modelo modelo = mapeadorObjetos.readValue(plan.getDiagrama(), Modelo.class);
        ArrayList<Node> nodes = modelo.buscarNodo(task.getId());
        for (Node node : nodes) {
            node.setProducto(null);
            node.setCargo(null);
            modelo.updateNode(node);
        }
        plan.setDiagrama(mapeadorObjetos.writeValueAsString(modelo));
    ...
    private void ejecutarExistenciaModelo(Plan plan, PlTask tarea, Position position, Set<CriticalyLevel> criticalyLevels, Modelo modelo, boolean tranversal, boolean existeAgrupado, boolean eraTranversal, boolean eraHito, boolean esHito) {
        if (tranversal) {
            TreeSet<CriticalyLevel> levels = new TreeSet<>(criticalyLevels);
        ...
        if (eraHito) {
            ArrayList<Node> nodes = modelo.buscarNodo(tarea.getId());
            Optional<CriticalyLevel> first = criticalyLevels.parallelStream().findFirst();
            for (Node node : nodes) {
                node.setCategory("");
                node.setColor("trasnparent");
                node.setColorBorde(first.isPresent() ? first.get().getColor() : "transparent");
                modelo.updateNode(node);
            }
            plan.setDiagrama(mapeadorObjetos.writeValueAsString(modelo));
            planService.saveAndFlush(plan);
        ...
        if (esHito) {
            ...
            Optional<CriticalyLevel> first = criticalyLevels.parallelStream().findFirst();
            for (Node node : nodes) {
                node.setCategory("Hito");
                node.setColor(first.isPresent() ? first.get().getColor() : "transparent");
                modelo.updateNode(node);
            }
            plan.setDiagrama(mapeadorObjetos.writeValueAsString(modelo));
            planService.saveAndFlush(plan);
    ...
    @PreAuthorize(value = "isFullyAuthenticated()")
    @GetMapping(value = "/tareasUsuario")
    public ModelAndView misTareas(PlTaskFilter filter, Boolean equipo, @AuthenticationPrincipal Users usuario, ModelMap map) {
