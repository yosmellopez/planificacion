...
public class PositionController {
    ...
    @PutMapping(value = "/guardarCargo/{cargoId}")
    @ApiOperation(value = "Returns the list of Position instances.")
    public ModelAndView guardarCargo(@RequestBody Position position, @PathVariable("cargoId") Position positionBd, ModelMap map) {
        Set<Management> direcciones = position.getDirecciones();
        positionBd.getDirecciones().clear();
        for (Management direccion : direcciones) {
            if (!positionBd.getDirecciones().contains(direccion)) {
                positionBd.getDirecciones().add(managementService.findOne(direccion.getId()));
            }
        }
        positionBd.clonarDatos(position);
        positionService.saveAndFlush(positionBd);
        LOGGER.debug("Rendering Positions list");
        map.put("success", true);
        map.put("message", "La operación se realizó correctamente");
        return new RestModelAndView(map);
