...
var TableEditableEstadoTarea = function (scopeAngular) {

    var oTable;
    var rowDelete = null;
    var cont = 0;
    var formTitle = "¿Deseas crear un nuevo estado de tarea? Sigue los siguientes pasos:";
    var scopeAng;

    //Inicializa la tabla
    var initTable = function () {
        var table = $('#estado-tarea-table-editable');

        var order = [[1, "asc"]];
        var aoColumns = [
            {"bSortable": false, "sWidth": '1%', "sClass": 'text-center'},
            {"bSortable": true, "sWidth": '20%', "sClass": 'form-control-text'},
            {"bSortable": true, "sWidth": '40%', "sClass": 'form-control-text'},
        ...
        oTable.init({
            ...
            dataTable: {
                ...
                "ajax": {
                    "url": "estado-tarea/listarEstado", // ajax source
                },
                "order": order,
                responsive: {
                    details: {}
    ...
    var btnClickModalEliminar = function () {
        ...
        Metronic.blockUI({target: '#estado-tarea-table-editable', animate: true});

        $.ajax({
            type: "POST",
            url: "estado-tarea/eliminarEstado",
            dataType: "json",
            data: {
                'estado_id': estado_id
            },
            success: function (response) {
                Metronic.unblockUI('#estado-tarea-table-editable');
                if (response.success) {
    ...
    var btnClickModalEliminarSeleccion = function () {
        ...
        $.ajax({
            type: "POST",
            url: "estado-tarea/eliminarEstados",
            dataType: "json",
    ...
    var btnClickSalvarForm = function (color) {
    ...
    var initAccionEditar = function () {
        $(document).on('click', "#estado-tarea-table-editable a.edit", function (e) {
            e.preventDefault();
            resetForms();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        ...
                        $('#nombre').val(response.estado.nombre);
                        $('#peso').val(response.estado.peso);
                        $('#color').val(response.estado.color);
                        $('#color').css("background-color", response.estado.color);
                        scopeAng.var = response.estado.color;
                        ...
                        formTitle = "¿Deseas actualizar el estado de tarea \"" + descripcion + "\" ? Sigue los siguientes pasos:";
                        $('#form-estado-title').html(formTitle);
    ...
    var initAccionEliminar = function () {
        $(document).on('click', "#estado-tarea-table-editable a.delete", function (e) {
            e.preventDefault();
